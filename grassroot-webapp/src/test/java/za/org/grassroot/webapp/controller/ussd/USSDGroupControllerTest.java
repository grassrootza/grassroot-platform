...
public class USSDGroupControllerTest extends USSDAbstractUnitTest {
    ...
    public void renameConfirmShouldWork() throws Exception {
        ...
        verifyNoMoreInteractions(userManagementServiceMock);
        verify(groupManagementServiceMock, times(1)).renameGroup(testGroup.getUid(), "a renamed test group", testUserUid);
        verifyNoMoreInteractions(groupManagementServiceMock);
        verifyZeroInteractions(eventManagementServiceMock);
    }
    ...
    public void consolidateGroupDoneScreenShouldWork() throws Exception {
        ...
        mergingGroup.setId(2L);
        when(userManagementServiceMock.findByInputNumber(testUserPhone, null)).thenReturn(testUser);
        when(groupManagementServiceMock.mergeGroups(testGroup.getId(), mergingGroup.getId(), testUser.getId(),
                                                    false, false, false)).thenReturn(mergingGroup);
        ...
        verify(userManagementServiceMock, times(1)).findByInputNumber(testUserPhone, null);
        verifyNoMoreInteractions(userManagementServiceMock);
        verify(groupManagementServiceMock, times(1)).mergeGroups(testGroup.getId(), mergingGroup.getId(), testUser.getId(), false, false, false);
        verifyNoMoreInteractions(groupManagementServiceMock);
        verifyZeroInteractions(eventManagementServiceMock);
    }
    ...
    public void newGroupCreateShouldWork() throws Exception {
        ...
        when(userManagementServiceMock.findByInputNumber(testUserPhone)).thenReturn(testUser);
        when(groupBrokerMock.create(testUserUid, nameToPass, null, testMembers, template)).thenReturn(testGroup);
