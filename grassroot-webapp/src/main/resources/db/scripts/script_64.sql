...
ALTER TABLE event DROP COLUMN send_blocked;

ALTER TABLE event ADD COLUMN type varchar(50);
UPDATE event SET type = 'MEETING' where event_type = 0;
...
ALTER TABLE event ADD COLUMN scheduled_reminder_time timestamp;
ALTER TABLE event ADD COLUMN scheduled_reminder_active boolean not null default false;
