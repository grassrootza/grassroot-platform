...
public class CreateJwtTokenRequest {
    ...
    public CreateJwtTokenRequest(JwtType jwtType, User user) {
        ...
        claims.put(JwtService.SYSTEM_ROLE_KEY, user.getStandardRoles().stream().map(role -> role.getName().name()).collect(Collectors.joining(",")));
    }
    ...
    public static CreateJwtTokenRequest makeSystemToken() {
        ...
        request.claims.put(JwtService.SYSTEM_ROLE_KEY, RoleName.ROLE_SYSTEM_CALL);
        return request;
    }
