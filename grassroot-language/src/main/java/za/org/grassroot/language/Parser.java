...
public class Parser {
    ...
    public List<DateGroup> parse(String value, Date referenceDate) {
        _logger.debug("Inside the parser, referenceDate={}", referenceDate);
        ...
        _logger.debug("We're back from collecting our global token streams, this is what we got {}", streams);
        ...
        for (TokenStream stream : streams) {
            ...
            _logger.debug("There is your group's date token part: " + (group != null ? group.getDates() : "null"));
    ...
    private DateGroup singleParse(TokenStream stream, String fullText, Date referenceDate) {
        ...
        _logger.debug("singleParse... token stream=: " + (stream.size() > 0 ? stream.get(0).getInputStream() : "null"));
        List<Token> tokens = ((NattyTokenSource) stream.getTokenSource()).getTokens();
        _logger.debug("singleParse.... list of tokens={}", tokens.isEmpty() ? "" : tokens.get(0).getInputStream());
