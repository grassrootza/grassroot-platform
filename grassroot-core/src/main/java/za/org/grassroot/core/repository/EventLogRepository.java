...
public interface EventLogRepository extends JpaRepository<EventLog, Long> {
    ...
    */
    ...
    @Query("SELECT CASE WHEN COUNT(e) > 0 THEN 'true' ELSE 'false' END FROM EventLog e WHERE e.event = ?1 and e.user = ?2 and e.eventLogType = za.org.grassroot.core.enums.EventLogType.EventChange")
    public Boolean changeNotificationSent(Event event, User user);
    ...
    */
     ...
     */
    @Query("SELECT CASE WHEN COUNT(e) > 0 THEN 'true' ELSE 'false' END FROM EventLog e WHERE e.event = ?1 and e.user = ?2 and e.eventLogType = za.org.grassroot.core.enums.EventLogType.EventRSVP AND e.message = 'Yes'")
    public boolean rsvpYesForEvent(Event event, User user);
